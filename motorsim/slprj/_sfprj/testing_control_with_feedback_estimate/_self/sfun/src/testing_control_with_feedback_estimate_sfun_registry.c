...
{
  extern unsigned int
    sf_testing_control_with_feedback_estimate_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_testing_control_with_feedback_estimate_process_check_sum_call(nlhs,plhs,
       nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_testing_control_with_feedback_estimate_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "testing_control_with_feedback_estimate") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_testing_control_with_feedback_estimate_autoinheritance_info(nlhs,
      plhs,3,newRhs);
  }
...
{
  extern unsigned int
    sf_testing_control_with_feedback_estimate_get_eml_resolved_functions_info
...
extern unsigned int sf_testing_control_with_feedback_estimate_method_dispatcher
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"testing_control_with_feedback_estimate")) {
    return(sf_testing_control_with_feedback_estimate_method_dispatcher
...
extern void testing_control_with_feedback_estimate_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    testing_control_with_feedback_estimate_terminator();
    sf_debug_terminate();
  }
...
extern void testing_control_with_feedback_estimate_initializer(void);
extern void testing_control_with_feedback_estimate_register_exported_symbols
  (SimStruct* S);
extern void testing_control_with_feedback_estimate_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  testing_control_with_feedback_estimate_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      testing_control_with_feedback_estimate_debug_initialize();
    }

    testing_control_with_feedback_estimate_initializer();
  }
